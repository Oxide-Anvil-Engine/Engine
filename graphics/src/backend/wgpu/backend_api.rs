...
use crate::backend::backend_trait::BackendOptions;
use crate::backend::wgpu::camera::CameraWGPU;
use crate::backend::wgpu::mesh_cache::MeshWGPU;
use crate::types::id::{GlobalCameraId, GlobalMeshId, GlobalPipelineId, GlobalRenderTargetId};
use wgpu::RenderPipeline as PipelineWGPU;
...
impl Backend<'static> for BackendWGPU {
    ...
    fn draw_passes(&mut self, frame: &mut FrameCtx, passes: &[RenderPass]) {
        ...
        for p in passes {
            ...
            let load_color = match desc.load_color {
                crate::render::pass::LoadAction::Clear => {
                ...
                crate::render::pass::LoadAction::Load => wgpu::LoadOp::Load,
            };
            ...
            let depth_ops = desc.clear_depth.map(|d| {
                let load = match desc.load_depth {
                    crate::render::pass::LoadAction::Clear => wgpu::LoadOp::Clear(d),
                    crate::render::pass::LoadAction::Load => wgpu::LoadOp::Load,
                };
                wgpu::Operations {
                    load,
    ...
    fn ensure_camera(
        &mut self,
        id: crate::types::id::GlobalCameraId,
        data: &crate::scene::camera::Camera,
    ) {
    ...
    fn ensure_mesh(
        &mut self,
        _id: crate::types::id::GlobalMeshId,
        _data: &crate::resources::data::mesh::MeshData,
    ) {
    }

    fn ensure_pipeline(
        &mut self,
        _id: crate::types::id::GlobalPipelineId,
        _data: &crate::resources::desc::pipeline::PipelineDesc,
    ) {
    }

    fn ensure_texture(
        &mut self,
        _id: crate::types::id::GlobalTextureId,
        _data: &crate::resources::data::texture::TextureData,
    ) {
    }
}

impl BackendWGPU {
    pub fn new(window: Arc<winit::window::Window>, opts: BackendOptionsWGPU) -> Self {
